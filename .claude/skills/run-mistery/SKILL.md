...
> **对应版本：v4 晨露胶片 morning dew film 视觉系统（八屏 + 真实管线 B + 语音 C + 降落音 D′：ADHD 舒缓环境声）** ｜ 迭代后请更新此行与下方内容，并在 `docs/CHANGELOG.md` 记一笔。
...
- **降落音（D′）**：降落屏（⑧）自动循环播一段**很安静的环境声**（取代旧的 `say` 朗读）——`assets/` 里放了 CC0 音频（`landing.mp3`/`drip.mp3`/`water.mp3`…）就用它，没放则用 numpy 合成的**很安静的水滴声**兜底（稀疏水珠落在近乎静默之上，音量压得很低 peak≈0.24），**跨平台、免 key、不失声**。逻辑见 `src/focus_audio.py`。（曾用双耳节拍 binaural，反馈太吵已换成水滴；旧的 `say` 三段朗读 `src/tts.py` 保留为历史，已不在降落屏调用。）
...
1. **① 启动**：晨露胶片冷雾调 + 漂浮晨雾；品牌 logo（水波 M + 石落涟漪，透明描边版；favicon 用 `.jpeg`）+ **Lora 衬线**字标 MURMORA + 斜体副标「夜林如墨 池塘倒映整片森林」→ 点一下进入（真机为 2-3s 自动跳）。
...
5. **⑤ 输出**：右上角一键切换 **思维导图 ⇄ 夜间日志**。感性日志为「今夜封存」**四段**——你说了什么 / 抚慰疗愈 / 明日第一步 / 睡前落点（可引公版文学标来源，结尾「今晚已经被保存」）；理性视图为导图分支 + 明日第一步 → 点「🌲 封存今晚」。
6. **⑥ 评分**：贴近度滑条 + 多选「哪部分最贴近」+ 选填 → 点「沉入池底」**落库**（`ratings` 表）。
7. **⑦ 封存**：森林倒映池塘 + 涟漪「今晚已被池塘收下」→ 点「跟着呼吸，慢慢降落」。
8. **⑧ 降落**：4-1-6 呼吸圈 + **自动循环播放很安静的水滴声（`src/focus_audio.py`）** + 三段静默字幕 → 点「让屏幕暗下来」触发渐暗熄屏 +「晚安」。

> 通道决定默认输出视图：**感性→日志**，**理性→导图**（⑤ 屏仍可手动互切）。
> 内容来自真实管线（B）；每夜整理与评分**已落库** `data/mistery.db`（entries/threads/ratings），下次倾倒命中过往会出现「似曾相识」召回（G3）。`MURMORA_DB` 可覆盖库路径。余下里程见 `docs/TODO.md`。
